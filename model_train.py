...
def train_model():
    ...
    ignored_params = list(map(id, model.net2.parameters()))
    base_params = filter(lambda p: id(p) not in ignored_params,
                         model.parameters())
    optimizer = optim.Adam([
        {'params': base_params}, 
        {'params': model.net2.parameters(), 'lr': 1e-2} #2
    ], lr=1e-4) # 4
    model.cuda()
    ...
    torch.save(model.cuda(),'Meta_train.pt')
...
def load_data(mod = 'train', dataset = 'tid2013', worker_idx = 0):
    ...
    if dataset == 'tid2013':
        data_dir = os.path.join('D:/dataset/IQA/tid2013/')
        worker_orignal = pd.read_csv(os.path.join(data_dir, 'image_labeled_by_per_noise.csv'), sep=',')
        image_path = 'D:/dataset/IQA/tid2013/distorted_images/'
    else:
        data_dir = os.path.join('D:/dataset/IQA/kadid10k/')
        worker_orignal = pd.read_csv(os.path.join(data_dir, 'image_labeled_by_per_noise.csv'), sep=',')
        image_path = 'D:/dataset/IQA/kadid10k/images/'
    workers_fold = "noise/"
    ...
    if mod == 'train':
        ...
        cross_data_path = 'D:/dataset/IQA/ChallengeDB_release/image_labeled_by_score.csv'
    ...
    return dataloader_train, dataloader_valid ## dataloader_train => support set, dataloader_valid => query set
